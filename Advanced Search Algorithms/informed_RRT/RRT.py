...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            x_center = np.array([int((self.start.col + self.goal.col)/2), int((self.start.row + self.goal.row)/2)])

            # Compute rotation matrix from elipse to world frame - C
            C = self.rotation_to_world_frame(c_min)
    ...
    def rotation_to_world_frame(self, c_min):
        E1 = (self.goal.col - self.start.col, self.goal.row - self.start.row)/c_min   # Transverse axis of ellipsoid
        W1 = [1, 0]     # First basis vector of world frame
        M = np.outer(E1, W1)   # Outer product
        U, S, V = np.linalg.svd(M) # SVD
        middleM = np.eye(2)  # Middle diagonal matrix
    ...
    def informed_RRT_star(self, n_pts=1000, neighbor_size=20):
        ...
        for i in range(n_pts):
            ...
            c = self.path_cost(self.start, self.goal)

            if c == 0:
                pass
            elif c != 0 and c_best == 0:
                c_best = c
            elif c < c_best and c_best != 0:
                c_best = c
